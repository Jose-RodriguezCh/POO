...
class view:
    ...
    def consultar_autos(ventana):
        ...
        if len(registros)>0:
            ...
            ctk.CTkLabel(ventana,text_color="#000000", text="No hay datos para mostrar", font=ctk.CTkFont(size=16)).pack(pady=20)
    ...
    def cambiar_autos(ventana,id_):
        ...
        if registro is None:
            messagebox.showinfo(icon="info",message="No existen este auto en la BD ...")
        else:
            view.borrarPantalla(ventana)
            ...
            lbl_titulo=Label(scrollbar,text=f".::Cambiar un Auto::.")
            lbl_titulo.pack(pady=10)
    ...
    def insertar_camionetas(ventana):
        ...
        lbl_cerrada=Label(scrollbar,text="¿Cerrada? (SI/NO):",justify=CENTER)
        lbl_cerrada.pack(pady=10)
        cerrada=StringVar()
        txt_cerrada=Entry(scrollbar,textvariable=cerrada,justify=RIGHT)
        txt_cerrada.pack(pady=5)
        
        btn_guardar=Button(scrollbar,text="Guardar",command=lambda:cont.controlador.insertar_camionetas(marca.get(),color.get(),modelo.get(),velocidad.get(),potencia.get(),plazas.get(),traccion.get(),cerrada.get()),justify=CENTER)
        btn_guardar.pack(pady=5)
    ...
    def consultar_camionetas(ventana):
        ...
        registros=cont.controlador.consultar_camionetas()
        if len(registros)>0:
            ...
            for item in registros:
                if item[8]==0:
                    ...
                    res="SI"
                fila = (item[0], item[1], item[2], item[3], item[4], item[5], item[6],item[7],res)
    ...
    def cambiar_camionetas(ventana,id_):
        registro=cont.controlador.consultar_id_camionetas(id_)
        if registro is None:
            ...
            Button(scrollbar,text="Guardar",command=lambda:cont.controlador.actualizar_camionetas(id_,marca.get(),color.get(),modelo.get(),velocidad.get(),potencia.get(),plazas.get(),traccion.get(),cerrada.get())).pack(pady=5)
    ...
    def eliminar_camionetas(ventana,id_):
        ...
        if registro is None:
            ...
            btn_eliminar=Button(ventana,text="Eliminar",command=lambda:cont.controlador.eliminar_camionetas(id_))
            btn_eliminar.pack(pady=5)
    ...
    def insertar_camiones(ventana):
        ...
        btn_guardar=Button(scrollbar,text="Guardar",command=lambda:cont.controlador.insertar_camiones(marca.get(),color.get(),modelo.get(),velocidad.get(),caballaje.get(),plazas.get(),ejes.get(),capacidad.get()),justify=CENTER)
        btn_guardar.pack(pady=5)
    ...
    def consultar_camiones(ventana):
        ...
        registros=cont.controlador.consultar_camiones()
        if len(registros)>0:
    ...
    def cambiar_camiones(ventana,id_):
        registro=cont.controlador.consultar_id_camiones(id_)
        if registro is None:
            ...
            ejes=StringVar()
            lbl_ejes=Entry(scrollbar,textvariable=ejes,justify=RIGHT)
            ejes.set(registro[7])
            lbl_ejes.pack(pady=5)

            Label(scrollbar,text="Capcidad de carga: ").pack(pady=5)
            capacidad=StringVar()
            lbl_capacidad=Entry(scrollbar,textvariable=capacidad,justify=RIGHT)
            capacidad.set(registro[8])
            lbl_capacidad.pack(pady=5)
                
            Button(scrollbar,text="Guardar",command=lambda:cont.controlador.actualizar_camiones(id_,marca.get(),color.get(),modelo.get(),velocidad.get(),potencia.get(),plazas.get(),ejes.get(),capacidad.get())).pack(pady=5)
    ...
    def eliminar_camiones(ventana,id_):
        ...
        if registro is None:
            ...
            btn_eliminar=Button(ventana,text="Eliminar",command=lambda:cont.controlador.eliminar_camiones(id_))
            btn_eliminar.pack(pady=5)
